...
public class Rocket extends MovableObject {
    ...
    public Rocket(int x, int y, int rotation, Map map) {
        super(Resources.getImage(Resources.Images.ROCKET), x, y, rotation);
        this.map = map;
        speed = 400;
        ...
        if (other.isMovable()) {
            ...
            if (other.getClass() == WallSolid.class) {
                map.addMovable(new Explosion(
                        Resources.getImage(
                                Resources.Images.EXPLOSION_LARGE),x-16, y-16, map));
                destroy();
            }
            if (other.getClass() == WallBreakable.class) {
                map.addMovable(new Explosion(
                        Resources.getImage(Resources.Images.EXPLOSION_LARGE),
                        (int) other.getCollisionMask().getBounds2D().getMinX()-16,
                        (int) other.getCollisionMask().getBounds2D().getMinY()-16,
                        map));
